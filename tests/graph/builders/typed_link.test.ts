...
describe("typed_link builder — Dataview branch", () => {
	...
	test("Link object → edge created", async (t) => {
		const pages = [dv_page("a.md", { up: { path: "b.md" } })];
		const r = await _add_explicit_edges_typed_link(plugin(), make_dv_files(pages));
		t.expect(r.edges).toHaveLength(1);
		t.expect(r.edges[0]!.source).toBe("a.md");
		t.expect(r.edges[0]!.edge_type).toBe("up");
		t.expect(r.edges[0]!.edge_source).toBe("typed_link");
	...
	test("markdown link string → edge created", async (t) => {
	...
	test("array of Link objects → one edge per link", async (t) => {
		const pages = [
	...
	test("non-link string (no markdown link pattern) → silently skipped", async (t) => {
	...
	test("invalid value type (number) → error", async (t) => {
	...
	test("LuxonDateTime-like value → error", async (t) => {
	...
	test("null/falsy value → skipped, no error", async (t) => {
	...
	test("when obsidian branch active, skips fields already in frontmatter", async (t) => {
		// 'up' is in frontmatter → dataview should skip it (no duplicate edge)
		const pages = [dv_page("a.md", { up: { path: "b.md" } }, { up: "[[b]]" })];
		const r = await _add_explicit_edges_typed_link(plugin(), make_dv_files(pages, true));
		// obsidian branch has no frontmatterLinks, so 0 from there
		// dataview branch skips 'up' because it's in frontmatter_keys
		t.expect(r.edges).toHaveLength(0);
	});

	test("when obsidian branch null, dataview processes frontmatter fields", async (t) => {
		// obsidian: null → frontmatter_keys is null → dataview processes all
		const pages = [dv_page("a.md", { up: { path: "b.md" } }, { up: "[[b]]" })];
		const r = await _add_explicit_edges_typed_link(plugin(), make_dv_files(pages, false));
		t.expect(r.edges).toHaveLength(1);
	});

	test("'file' and 'aliases' fields always skipped", async (t) => {
